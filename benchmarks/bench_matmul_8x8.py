"""Test 8x8 register tiling with transposed tileA."""
import sys
import time
import struct
import numpy as np
...
# 8x8 register tile, 16x16 workgroup = 128x128 block
# tileA transposed: [BK][BM], tileB: [BK][BN]
# Shared: 16*128 = 2048 each = 4096 total = 16KB (well within 48KB)
SHADER_8x8 = """
const WG: u32 = 16u;
const TM: u32 = 8u;
const TN: u32 = 8u;
...
var<workgroup> tileA: array<f32, 1024>;  // BK * BM = 8 * 128
var<workgroup> tileB: array<f32, 1024>;  // BK * BN = 8 * 128
...
fn main(
    ...
    for (var t: u32 = 0u; t < numTiles; t++) {
        ...
        // Load tileA (transposed): 128 rows × 8 cols = 1024 elements, 256 threads -> 4 each
        ...
        // Load tileB: 8 rows × 128 cols = 1024
        for (var load = 0u; load < 4u; load++) {
            let flat = load * 256u + tid;
            let b_r = flat / BN;
            let b_c = flat % BN;
...
def setup(device, shader, M, N, K, A, B, BM=128, BN=128):
    ...
    buf_p = device.create_buffer_with_data(data=params, usage=wgpu.BufferUsage.UNIFORM)
...
def main():
    ...
    for M in [2048, 4096]:
        ...
        ctx = setup(device, SHADER_8x8, M, M, M, A, B)
