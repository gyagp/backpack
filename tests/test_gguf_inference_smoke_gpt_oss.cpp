...
static std::string find_model() {
    const char* env = std::getenv("GPT_OSS_GGUF_PATH");
...
int main() {
    ...
    if (model_path.empty()) {
        std::cout << "SKIP: gpt-oss-20b GGUF not found. Set GPT_OSS_GGUF_PATH or place in models/." << std::endl;
        return 0;
    }

    {
        std::cout << "Test: gpt-oss-20b 16-token generation (D3D12)... ";
