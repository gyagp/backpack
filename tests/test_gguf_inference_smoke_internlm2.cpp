...
static std::string find_model() {
    const char* env = std::getenv("INTERNLM2_GGUF_PATH");
    if (env && std::filesystem::exists(env)) return env;

    std::vector<std::string> candidates = {
        "models/internlm2-chat-1_8b.Q4_K_M.gguf",
        "models/internlm2-chat-1_8b-Q4_K_M.gguf",
        "models/InternLM2-Chat-1.8B.Q4_K_M.gguf",
        "models/internlm2-chat-1.8b.Q4_K_M.gguf",
        "../models/internlm2-chat-1_8b.Q4_K_M.gguf",
    };
...
int main() {
    ...
    if (model_path.empty()) {
        std::cout << "SKIP: internlm2-chat-1_8b GGUF not found. Set INTERNLM2_GGUF_PATH or place in models/." << std::endl;
        return 0;
    }

    {
        std::cout << "Test: internlm2-chat-1_8b 16-token generation (D3D12)... ";
