...
static void build_random_q8_0_block(uint8_t* block, std::mt19937& rng) {
...
int main() {
    ...
    constexpr int TOTAL_FLOATS = N_BLOCKS * Q8_0_BLOCK_SIZE;

    std::mt19937 rng(42);
    std::vector<uint8_t> quant_data(N_BLOCKS * Q8_0_BYTES_PER_BLOCK);
    for (int i = 0; i < N_BLOCKS; i++) {
        build_random_q8_0_block(quant_data.data() + i * Q8_0_BYTES_PER_BLOCK, rng);
    }

    std::vector<float> cpu_out = cpu_dequant_q8_0_n(quant_data.data(), N_BLOCKS);

    auto ctx = create_gpu_context();
    std::printf("GPU test: Q8_0 dequant, %d blocks (%d values)\n", N_BLOCKS, TOTAL_FLOATS);
