...
static std::string find_model() {
    const char* env = std::getenv("GRANITE_GGUF_PATH");
    if (env && std::filesystem::exists(env)) return env;

    std::vector<std::string> candidates = {
        "models/granite-3.1-2b-instruct.Q4_K_M.gguf",
        "models/granite-3.1-2b-instruct-Q4_K_M.gguf",
        "models/Granite-3.1-2B-Instruct.Q4_K_M.gguf",
        "../models/granite-3.1-2b-instruct.Q4_K_M.gguf",
    };
...
int main() {
    ...
    if (model_path.empty()) {
        std::cout << "SKIP: granite-3.1-2b-instruct GGUF not found. Set GRANITE_GGUF_PATH or place in models/." << std::endl;
        return 0;
    }

    {
        std::cout << "Test: granite-3.1-2b-instruct 16-token generation (D3D12)... ";
