...
bool ModelRunner::loadOnnx(GPUContext& ctx, const std::string& onnxDir) {
    ...
    for (uint32_t i = 0; i < cfg.nLayer; i++) {
        const bool qwenLayer = cfg.arch == "qwen35" && i < onnx.layers.size();
        const bool qwenAttention = qwenLayer && cfg.isAttentionLayer(i) &&
            onnx.layers[i].qwenAttnQ.N && onnx.layers[i].qwenAttnK.N &&
            onnx.layers[i].qwenAttnV.N && onnx.layers[i].o.N;
        const bool qwenSsm = qwenLayer && !cfg.isAttentionLayer(i) &&
            onnx.layers[i].qwenSsmQkv.N && onnx.layers[i].qwenSsmZ.N &&
            onnx.layers[i].qwenSsmBeta.N && onnx.layers[i].qwenSsmAlpha.N &&
            onnx.layers[i].qwenSsmOut.N && !onnx.layers[i].qwenSsmConvWeight.empty() &&
            !onnx.layers[i].qwenSsmDtBias.empty() && !onnx.layers[i].qwenSsmA.empty() &&
            !onnx.layers[i].qwenSsmNorm.empty();
        const bool attentionReady = qwenLayer ? (qwenAttention || qwenSsm) :
            ((onnx.layers[i].qkv.N != 0 || onnx.layers[i].qOnly.N != 0) &&
             onnx.layers[i].o.N != 0);
        if (i >= onnx.layers.size() || !attentionReady ||
            onnx.layers[i].gateup.N == 0 || onnx.layers[i].down.N == 0) {
            fprintf(stderr,
    ...
    for (uint32_t i = 0; i < cfg.nLayer; i++) {
        ...
        if (cfg.arch == "qwen35" && cfg.isAttentionLayer(i)) {
            ...
            betaAlpha.K = ld.qwenSsmBeta.K;
            betaAlpha.weights = ld.qwenSsmBeta.weights;
            betaAlpha.weights.insert(betaAlpha.weights.end(),
                ld.qwenSsmAlpha.weights.begin(), ld.qwenSsmAlpha.weights.end());
            ...
            uploadRaw(p + "conv", ld.qwenSsmConvWeight, lw.ssmConv1dW);
            uploadRaw(p + "conv_bias", ld.qwenSsmConvBias, lw.ssmConv1dBias);
            uploadRaw(p + "dt_bias", ld.qwenSsmDtBias, lw.ssmDtBias);
            uploadRaw(p + "a", ld.qwenSsmA, lw.ssmA);
            uploadRaw(p + "norm", ld.qwenSsmNorm, lw.ssmNorm);
        }
...
void ModelRunner::loadWeights(const GGUFFile& gguf,
    ...
    auto uploadQ4Dense = [&](const std::string& name, const KQuantPacked& kq,
                             GPUBuffer& weights, GPUBuffer& scalesMins) {
        // The extra resident layout is beneficial for the 32-layer Qwen 4B
        // tuple, but pressures the validated 2B decode working set. Keep the
        // optimization model-specific until a disposable prefill cache exists.
        if (cfg.arch != "qwen35" || cfg.nLayer != 32u || cfg.nEmbd != 2560u ||
        ...
        if (isQ35AttnLayer) {
            ...
            if (qi != gguf.tensor_index.end() && ki != gguf.tensor_index.end() && vi != gguf.tensor_index.end()) {
                ...
                auto loadExact = [&](const GGUFTensorInfo& t, uint32_t N,
                                     ...
                                     GPUBuffer& q4Dense, GPUBuffer& q4ScaleMin,
                ...
                loadExact(kt,kOutDim,".k_kq",lw.kSepKQ,lw.kSepQ4Dense,lw.kSepQ4ScaleMin,lw.kSepKQType,lw.kSepKQNBlocks,lw.kSepKQRowStride,lw.kSepW,lw.kSepS);
                loadExact(vt,vOutDim,".v_kq",lw.vSepKQ,lw.vSepQ4Dense,lw.vSepQ4ScaleMin,lw.vSepKQType,lw.vSepKQNBlocks,lw.vSepKQRowStride,lw.vSepW,lw.vSepS);
                if (i == 3) {
        ...
        if (!isQ35AttnLayer) {
        {
            ...
            if (qi == gguf.tensor_index.end() && qkvi_pre != gguf.tensor_index.end()) {
                ...
                if (tensorIsKQ) {
                    ...
                    if (lw.qkvKQType == GGUF_TYPE_Q4_K)
                ...
                if (ki == gguf.tensor_index.end() || vi == gguf.tensor_index.end()) {
                    ...
                    if (allKQ) {
                        ...
                        if ((GGUFType)qt.type == GGUF_TYPE_Q4_K)
                            uploadQ4Dense("L" + std::to_string(i) + ".qkv_ort", fused, lw.qkvQ4Dense, lw.qkvQ4ScaleMin);
                    } else {
            ...
            if (it != gguf.tensor_index.end()) {
                auto& ti = gguf.tensors[it->second];
                    ...
                    if (tensorIsKQ) {
                        ...
                        if (lw.oKQType == GGUF_TYPE_Q4_K)
                            uploadQ4Dense("L" + std::to_string(i) + ".o_ort", kq, lw.oQ4Dense, lw.oQ4ScaleMin);
                    } else {
        ...
        // Fuse gate + up
        {
            ...
            if (gi != gguf.tensor_index.end() && ui != gguf.tensor_index.end()) {
                ...
                if (bothKQ) {
                    ...
                    if (lw.guKQType == GGUF_TYPE_Q4_K)
                        uploadQ4Dense("L" + std::to_string(i) + ".gu_ort", fused, lw.guQ4Dense, lw.guQ4ScaleMin);
                } else {
...
fn main(@builtin(workgroup_id) wid: vec3<u32>,
    ...
    if (kind == 0u) { Q[h * dk + d] = value * inv; }
                ...
                auto bg = makeBG(plConvSplit, {
                    ...
                    {3, convBias}, {4, q35SsmQBuf}, {5, q35SsmKBuf},
                    {6, q35SsmVBuf}, {7, p}});
            ...
            } else {
            {
                ...
                auto bg = makeBG(plConvSilu, {
                    {0, ssmConvState[i]}, {1, qkvBuf}, {2, lw.ssmConv1dW},
                    {3, convBias}, {4, q35ConvOutBuf}, {5, p}});
...
void ModelRunner::initQwen35PrefillResources() {
    ...
    const uint64_t q6ScaleGroup=enableQ6Batched?4u:32u;
    qwen35Pf.kqActScale=mk("qpf_kq_act_scales",(uint64_t)C*((maxK+q6ScaleGroup-1u)/q6ScaleGroup));
    // AMD and Intel use disposable ORT-style tile buffers to avoid retaining
    // a second Q4_K weight copy and regressing decode through working-set
    // pressure. This path uses subgroup shuffles, not subgroup matrices.
    ...
    if(enableQ6Batched){
        (void)getKernel("q8_quantize_batched_dp4a_q6");
        (void)getKernel("q6k_matmul_prequant_batched_dp4a");
        if(gpu->adapterName.find("Intel")!=std::string::npos)
            (void)getKernel("q6k_matmul_prequant_batched_dp4a_reduc16");
    }
...
int32_t ModelRunner::prefillQwen35Batched(
    ...
    while(done<T){
        ...
        if(lmHeadKQ.handle){
            ...
            std::vector<float>emb((size_t)M*E);
            for(uint32_t t=0;t<M;t++)memcpy(emb.data()+(size_t)t*E,
                embeddingCPU.data()+(size_t)toks[t]*E,(size_t)E*sizeof(float));
        ...
        auto mmk=[&](GPUBuffer x,GPUBuffer w,GPUBuffer dense,GPUBuffer scaleMin,GGUFType t,uint32_t nb,uint32_t rs,GPUBuffer bias,GPUBuffer y,uint32_t K,uint32_t N,const std::string&n){
            ...
            if(q5ProjectionDense){
                ...
                add(*ortQ4Tile,{{0,qwen35Pf.kqActQ8},{1,qwen35Pf.kqActScale},
                    {2,dense},{3,scaleMin},{4,y},{5,p}},mt*nt,1,1,n+"_ort64_loadtime");
            }
            else if(t==GGUF_TYPE_Q4_K&&q4Repack&&ortQ4Tile){
                auto qp=mkp(n+"_ort_quant_p",{K,N,M,nb,rs});
                auto&quant=getKernel("q8_quantize_batched_dp4a");
                add(quant,{{0,x},{1,qwen35Pf.kqActQ8},{2,qwen35Pf.kqActScale},{3,qp}},(K+255)/256,M,1,n+"_quant");
            ...
            // Every chunk is full when the request-level gate above is true.
                ...
                add(quant,{{0,x},{1,qwen35Pf.kqActQ8},{2,qwen35Pf.kqActScale},{3,p}},(K+255)/256,M,1,n+"_quant");
                const bool intelQ6=gpu->adapterName.find("Intel")!=std::string::npos;
                auto&kp=getKernel(intelQ6?"q6k_matmul_prequant_batched_dp4a_reduc16":"q6k_matmul_prequant_batched_dp4a");
                add(kp,{{0,qwen35Pf.kqActQ8},{1,qwen35Pf.kqActScale},{2,w},{3,bias},{4,y},{5,p}},(M+7)/8,(N+(intelQ6?15u:7u))/(intelQ6?16u:8u),1,n);
            }
        ...
        auto proj=[&](GPUBuffer x,GPUBuffer kqw,GPUBuffer dense,GPUBuffer scaleMin,GGUFType type,uint32_t nb,uint32_t rs,
            ...
            if(kqw.handle)mmk(x,kqw,dense,scaleMin,type,nb,rs,bias,y,K,N,n);
            else mm(x,q8w,q8s,bias,y,K,N,n);
        };
        for(uint32_t li=0;li<cfg.nLayer;li++){
            ...
            if(!cfg.isAttentionLayer(li)){
                ...
                proj(qwen35Pf.norm,lw.qkvKQ,lw.qkvQ4Dense,lw.qkvQ4ScaleMin,lw.qkvKQType,lw.qkvKQNBlocks,lw.qkvKQRowStride,lw.qkvW,lw.qkvS,zeroBiasQKV,qwen35Pf.qkv,E,C,L+"qkv");
                ...
                GPUBuffer convBias=lw.ssmConv1dBias.handle?lw.ssmConv1dBias:zeroBiasQKV;
                add(convKernel,{{0,ssmConvState[li]},{1,qwen35Pf.qkv},{2,lw.ssmConv1dW},{3,convBias},{4,qwen35Pf.conv},{5,cp}},(C+255)/256,1,1,L+"conv");
                ...
                proj(qwen35Pf.norm,lw.qjKQ,lw.qjQ4Dense,lw.qjQ4ScaleMin,lw.qjKQType,lw.qjKQNBlocks,lw.qjKQRowStride,lw.qjW,lw.qjS,zeroBiasV,qwen35Pf.qj,E,2u*qd,L+"q");
                proj(qwen35Pf.norm,lw.kSepKQ,lw.kSepQ4Dense,lw.kSepQ4ScaleMin,lw.kSepKQType,lw.kSepKQNBlocks,lw.kSepKQRowStride,lw.kSepW,lw.kSepS,zeroBiasV,qwen35Pf.ak,E,kd,L+"k");
                proj(qwen35Pf.norm,lw.vSepKQ,lw.vSepQ4Dense,lw.vSepQ4ScaleMin,lw.vSepKQType,lw.vSepKQNBlocks,lw.vSepKQRowStride,lw.vSepW,lw.vSepS,zeroBiasV,qwen35Pf.av,E,kd,L+"v");
