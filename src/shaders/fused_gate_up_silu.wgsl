...
fn main(
    ...
    var accGate: array<f32, 64>;
    var accUp: array<f32, 64>;
    ...
    for (var t: u32 = 0u; t < numTiles; t = t + 1u) {
        ...
        // Load tileA from input (shared for gate and up projections)
        {
        ...
        // Gate projection
        {
            ...
            if global_b_row < params.K && global_b_col + 15u < params.N {
                ...
                unpack_tile_b(
                    b_r * BN + b_c0,
                    Wgate[b_idx / 2u], Wgate[(b_idx + 2u) / 2u],
                    Wgate[(b_idx + 4u) / 2u], Wgate[(b_idx + 6u) / 2u],
                    Wgate[(b_idx + 8u) / 2u], Wgate[(b_idx + 10u) / 2u],
                    Wgate[(b_idx + 12u) / 2u], Wgate[(b_idx + 14u) / 2u],
                );
            } else {
                for (var i = 0u; i < 16u; i++) {
                    let bc = b_c0 + i;
        ...
        workgroupBarrier();
        accumulate(a_base, b_base, &accGate);
        workgroupBarrier();

        // Up projection (tileA already loaded — reuse from memory)
        {
            let global_b_row = tile_k + b_r;
            let global_b_col = block_col + b_c0;
            if global_b_row < params.K && global_b_col + 15u < params.N {
                ...
                unpack_tile_b(
                    b_r * BN + b_c0,
                    Wup[b_idx / 2u], Wup[(b_idx + 2u) / 2u],
                    Wup[(b_idx + 4u) / 2u], Wup[(b_idx + 6u) / 2u],
                    Wup[(b_idx + 8u) / 2u], Wup[(b_idx + 10u) / 2u],
                    Wup[(b_idx + 12u) / 2u], Wup[(b_idx + 14u) / 2u],
                );
            } else {
                for (var i = 0u; i < 16u; i++) {
                    let bc = b_c0 + i;
        ...
        workgroupBarrier();
        accumulate(a_base, b_base, &accUp);
        workgroupBarrier();
    }
    ...
    for (var rm = 0u; rm < TM; rm++) {
        ...
        for (var rn = 0u; rn < TN; rn++) {
            ...
            if col < params.N {
                let g = accGate[rm * TN + rn];
                let silu_g = g / (1.0 + exp(-g));
                let u = accUp[rm * TN + rn];
                output[row_off + col] = silu_g * u;
            }
        }
    }
}
